...
echo "Look for: ${FRAMEWORKS_REPOSITORY}/ProjectWOnder/${WONDER_GIT_REFERENCE}/${WO_VERSION}"
if [ -e "${FRAMEWORKS_REPOSITORY}/ProjectWOnder/${WONDER_GIT_REFERENCE}/${WO_VERSION}" ]; then
	echo "${FRAMEWORKS_REPOSITORY}/ProjectWOnder/${WONDER_GIT_REFERENCE}/${WO_VERSION} Found!"
else
	echo "${FRAMEWORKS_REPOSITORY}/ProjectWOnder/${WONDER_GIT_REFERENCE}/${WO_VERSION} NOT found! You must build Wonder with the same WONDER_REVISION (${WONDER_REVISION}) and WO_VERSION (${WO_VERSION})"
	exit 1
fi
